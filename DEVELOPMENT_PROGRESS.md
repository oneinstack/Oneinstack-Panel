...
| 项目 | 仓库 | 基线提交 | 当前状态 |
| --- | --- | --- | --- |
| 后端 | `Oneinstack-Panel` | `5e74dac` | 0.4 上线工程与安全闭环持续开发中 |
| 前端 | `Oneinstack-Panel-Web` | `3055317a` | 已完成现代化界面、全局主题与安全防火墙控制台 |
...
### 4.1.1 2026-08-09 Nginx 卸载重装后网站配置恢复

状态：代码完成并部署测试环境，完整安装—卸载—重装矩阵待验收

完成内容：

- 明确以 Panel 数据库中的网站记录和结构化设置作为受管站点配置的可信来源。
- Nginx 或 OpenResty 安装成功后，自动扫描全部启用网站并重建缺失的虚拟主机配置；已有配置保持不变，停用网站不会被恢复。
- 网站生命周期巡检增加缺失配置自愈，能够修复升级前已经出现的“网站记录存在但 `.conf` 丢失”状态。
- Nginx `configure.sh` 会从最近一次安全卸载归档恢复原始 `.conf` 文件，保留用户通过配置编辑器进行的直接修改。
- Center 稳定 Nginx 组件包升级为 `1.0.3`，Panel 内置兜底包升级为 `1.0.1`，并重新生成文件摘要。

验证结果：

- 新增回归测试覆盖启用网站恢复、已有配置不覆盖、停用网站不恢复。
- 网站服务、软件任务处理器和脚本仓库 Go 测试通过。
- Panel 内置包和 Center Nginx 包的 SHA-256 完整性校验、Shell 语法检查通过。
- 测试服务器已升级到 `v0.1.0-test.56`；启动时自动恢复
  `192.168.1.6.conf` 和 `www.spdzy.com.conf`，`nginx -t` 与
  `/health/ready` 均通过。
- 最终 Linux amd64 测试二进制 SHA-256 为
  `97e16b1000899327a353e43dd509debbc5d10872ff2f83670a29c3b5a1c3180a`。
- Center 已发布 Nginx 稳定组件包 `1.0.3`，解析接口确认 Ubuntu 24.04
  amd64 会选择该版本。
- 测试服务器升级前文件已备份到
  `/usr/local/one/backups/manual-release-v0.1.0-test.56`。

### 4.1.2 2026-08-09 纯数字用户登录与错误提示修复

状态：代码完成并部署测试环境

完成内容：

- 统一用户创建和登录的账号格式规则，账号可由字母、数字、下划线和连字符组成，允许
  `123` 这样的纯数字账号正常登录。
- 登录时自动清理账号首尾空格；用户创建接口复用同一套服务端校验，避免再次出现
  “可以创建但无法登录”的账号。
- API 不再为空错误详情生成无意义的通用说明；前端错误解析优先显示明确的业务
  `message`，仅在旧接口没有业务消息时才回退到 `detail`。
- 保留新用户首次登录强制修改初始密码的安全流程。

验证结果：

- 新增纯数字账号登录、用户名格式和错误响应回归测试；Panel 全量 Go 测试通过。
- 前端错误解析 2 个 Vitest 用例、TypeScript 类型检查和生产构建通过。
- 新嵌入 WebUI SHA-256 为
  `07693ca7a6bc37508c52e367e94340194c36cf352c71fa11bbbc66b2788f098c`。
- Panel `v0.1.0-test.57` 已部署到 `192.168.1.6:8089`，服务和 ready 检查正常；
  账号 `123` 实际登录返回 `authenticated=true`，并按预期进入首次改密流程。
- 部署后二进制 SHA-256 为
...
鉴权、服务生命周期、健康探针、默认 IP/HTTP 与可选 HTTPS 面板访问、访问配置 systemd 自动应用/失败恢复、可信代理/配置事务、文件安全边界、回收站、容量保护、Web 终端低权限隔离、服务端会话吊销、首次登录强制改密、TOTP/恢复码、实例 JWT/凭据密钥、静态页面嵌入、安全安装器、持久化软件任务、四组件服务状态与结构化配置、配置发布历史与人工恢复、四组件服务健康告警、网站配置补偿事务、SSL/ACME、网站安全删除与整站备份、MySQL 数据库生命周期/备份恢复、计划任务安全模板与主动取消、防火墙安全闭环、签名更新/失败回滚、持久化防篡改审计、监控通知告警、实时运行日志和供应链发布制品已完成第一轮源码整改。发布密钥轮换运维、访问配置/防火墙/更新/低权限终端实机矩阵、真实公共 CA/Nginx/MySQL 矩阵和真实干净系统完整安装首轮验收等 P0/P1 问题仍未清除。

### 4.9 2026-07-29 组件配置历史与人工恢复

状态：前后端代码闭环完成，真实四组件恢复矩阵待验收

完成内容：

- 新增组件配置历史表，每次配置任务原子保存发布前、目标白名单参数、组件版本、基础指纹、任务、操作者和恢复来源。
- 配置历史状态与持久化软件任务同步，覆盖发布中、成功、失败、取消、停服中断和 Panel 重启核验，不会把失败发布误标为可恢复版本。
- 新增历史分页查询、恢复差异预览和创建恢复任务 API；只能恢复成功记录，并会按当前组件白名单重新校验旧参数。
- 手动恢复不直接写文件，而是重新进入组件互斥、版本冲突检查、语法校验、reload/restart、自动回滚、SSE 和审计链。
- 配置抽屉新增发布历史时间线，显示版本、变更数量、任务状态和恢复来源；恢复前二次确认并转到统一任务进度抽屉。
- 配置历史只保存结构化白名单值，不保存网站路径、监听地址、密码或任意配置文本。

验证结果：

- Panel 全量 Go 包测试通过；需要监听随机本地端口的 `server` 测试在授权环境单独通过。
- 新增配置历史分页/读取、成功任务落库和失败任务终态测试；当前共有 256 个 Go 测试函数。
- 前端 Vitest、TypeScript 类型检查和生产构建通过。
- 嵌入前端 `webui/app.zip` SHA-256 为
  `c088ba1a7b7e0fe0fa6c08b84fa8fcef4beff9e17cb13681eb509a6a18b3bf8b`。

待完成：

- 在真实 Nginx、MySQL 8、PHP-FPM 和 Redis 上验证多次发布、历史恢复、服务原本停止、配置冲突、语法失败与 Panel 重启矩阵。
- 面板访问配置的 systemd 自动应用/失败恢复代码已在下一批次完成。

### 4.10 2026-07-29 四组件服务健康告警

状态：前后端代码闭环完成，真实 systemd 长时间运行矩阵待验收

完成内容：

- 新增 Nginx、MySQL 8、PHP-FPM 和 Redis 服务健康采集器，只使用本地已验证的缓存包或内置包执行状态探针，Center 离线不会阻塞健康检查。
- 每分钟并行检查已安装组件的 systemd 加载、运行和子状态；安装、升级、卸载或服务操作进行中时标记为忙碌，不产生误告警。
- 连续两次异常才进入告警状态，恢复正常后自动生成恢复事件；持续异常每 30 分钟最多提醒一次，避免通知风暴。
- 服务异常事件复用现有监控 Webhook、加密凭据、投递记录、静默、恢复和审计链，并增加组件资源类型与组件标识。
- 新增组件健康列表、立即检查和按组件静默 API；监控摘要同时统计服务待确认与告警数量。
- 前端监控页新增组件健康卡片，显示安装版本、运行版本、systemd 状态、最近检查、错误原因、静默状态和手动复查入口。

验证结果：

- 组件健康状态机、连续失败、通知发送、恢复事件、静默抑制、监控摘要、未安装跳过、任务忙碌和本地包解析测试通过。
- Panel 定向测试、全量 Go 测试、`go vet`、生产构建、前端 TypeScript、Vitest 和生产构建通过。
- 当前共有 259 个 Go 测试函数；嵌入前端 `webui/app.zip` SHA-256 为
  `e4cc73a1cd858f47bc5996c3360826ec9c76bcf1c8c30b2e7f862a25e40e4167`。

待完成：

- 在 Ubuntu 22.04/24.04 真实 systemd 主机验证服务启动中、停止、失败、超时、Panel 重启、Center 中断和长时间运行矩阵。
- OpenResty、Java、phpMyAdmin 等后续组件需要在独立服务动作包稳定后接入同一健康告警协议。
- 面板访问配置的 systemd 自动应用、启动失败恢复已在下一批次完成。

### 4.11 2026-07-29 面板访问配置自动应用与失败恢复

状态：前后端源代码闭环完成；按本轮要求跳过测试、构建、部署和实机调试

完成内容：

- 面板访问配置保存时先保留权限为 `0600` 的旧配置快照，再原子写入候选配置，并创建独立持久化事务。
- 受管 Linux systemd 环境通过瞬态 `one-network-apply-*` 单元延迟重启 Panel，原 HTTP 响应有时间返回给前端。
- 外部应用任务同时检查 `one.service` 活跃状态和新 HTTP `/health/ready`；新端口、监听地址或证书导致启动失败时自动恢复旧配置并重新启动。
- 配置快照和事务均带 SHA-256 校验，事务 ID 只能映射到固定目录；跨进程文件锁避免应用任务、OnFailure 恢复和新进程提交状态互相覆盖。
- 安装器新增 `one-network-recover.service`，并由 `one.service` 的 `OnFailure` 触发；服务器在配置切换期间断电或启动失败时仍可恢复未完成事务。
- 新增访问配置事务状态 API，显示等待应用、应用中、成功、已恢复和失败状态；同一时间只允许一个访问配置事务。
- 前端设置页显示自动应用状态、恢复错误和手动重启降级提示；新 HTTP 地址通过就绪检查后自动跳转，失败时继续回到原地址。
- 非 systemd、非受管服务环境保留原有“保存后手动重启”模式，不执行危险的进程内自重启。

本轮未执行：

- 未运行 Go/前端测试、类型检查、构建、Shell 检查或嵌入包同步。
- 未部署到测试服务器，未执行端口切换、错误证书、systemd OnFailure、断电恢复或防火墙补偿实机矩阵。

待完成：

- 后续集中验证 Ubuntu 22.04/24.04 上端口切换、IPv4/IPv6、HTTPS、自签名证书、端口抢占、进程崩溃和断电恢复矩阵。
- 下一代码开发入口转为 Web 终端/高级 Shell 的低权限隔离、命令级审计和并发限制。

### 4.12 2026-07-30 Web 终端低权限隔离与审计

状态：前后端源代码实现完成；按当前要求未执行测试、构建、部署或联调

完成内容：

- Web 终端默认关闭；启用后不再继承 Panel 的 root 身份，统一通过独立
  `one-terminal` 系统用户启动交互式 Bash。
- 子进程使用可信绝对路径调用 `prlimit` 和 `setpriv`，清除继承、环境和
  capability 权限，设置 `no-new-privileges`，并限制进程数、文件句柄、
  文件大小、地址空间、CPU 时间、会话总时长和单会话输出总量。
- 启动前验证 Panel 管理进程身份、低权限用户 UID/GID/主目录、特权组和
  工作目录；隔离条件不完整时拒绝创建终端，不降级为 root Shell。
- 安装器创建不可登录的 `one-terminal` 系统用户和 `0750` 独立工作目录，
  并要求系统提供 `setpriv`、`prlimit`、`getent` 和 `nologin`。
- 新增全局和单账号并发配额、空闲超时、最大会话时长及活动会话查询；超限
  返回 429，不会继续创建 PTY。
- 一次性终端票据绑定来源 IP、浏览器 User-Agent、管理员安全版本和主登录
  会话；主会话被吊销、过期或改密后，已连接终端会自动关闭。
- 审计链新增终端会话建立、关闭和逐行命令提交事件；只保存输入长度、
  操作者和会话 ID，不保存命令名称、命令参数、完整命令、交互密码或终端
  输出，避免把终端内输入的敏感信息写入审计库。
- 上述为该阶段的原始审计策略；2026-08-08 已升级为保存脱敏后的可读指令，当前策略见
  “Root 终端可读指令审计”章节。
- 前端新增管理员“安全终端”入口、隔离能力状态、运行身份、时长和并发信息，
  连接前必须再次输入管理员密码；终端支持标准 PTY 输入、方向键、粘贴和
  自适应窗口尺寸。
- 配置新增 `terminalIdleMinutes`、`terminalMaxConcurrent`、
  `terminalMaxPerUser`、`terminalUser` 和 `terminalWorkingDirectory`，
  并支持对应环境变量和启动校验。

本轮未执行：

- 未运行 Go/前端测试、类型检查、构建、Shell 检查或嵌入包同步。
- 未部署测试服务器，未验证 Ubuntu/Rocky 上用户创建、PTY、systemd、
  并发、空闲关闭、主会话吊销和资源限制行为。

待完成：

- 在真实 Linux 主机完成上述安全矩阵后，才可将 Web 终端标记为可生产启用。
- 下一源码开发入口转为发布密钥轮换、撤销和可信密钥状态管理。
...
状态：自动应用和失败恢复源代码闭环完成，真实 systemd/代理矩阵待验收
...
- 新增外部 systemd 应用事务、健康确认、配置快照恢复、跨进程锁和断电后的 OnFailure 恢复单元。
- 前端可查看应用/恢复状态；新 HTTP 地址就绪后自动跳转，非受管环境明确降级为手动重启。
- 前端生产包已重新构建并同步到后端嵌入资源。

待完成：

- 需要在 Ubuntu 22.04/24.04 上验证公网 IP、IPv6、IP SAN 证书、自签名证书及 Nginx/Caddy 代理矩阵。
- 本轮按要求未构建前端嵌入包，需在集中验证阶段统一构建并同步。
...
- 四组件服务状态已接入监控告警；真实 systemd 长时间运行矩阵仍待验收。
...
- 配置历史列表、差异预览和管理员手动恢复已完成；真实四组件恢复矩阵仍待验收。
- 四组件服务健康告警已接入现有监控通知闭环；面板访问配置自动应用/失败恢复也已完成源代码闭环。
...
- 持久化审计日志、会话安全、监控通知告警、实时运行日志、计划任务安全模板、网站整站备份、面板访问配置及自动恢复、四组件服务控制、结构化配置发布、配置历史恢复、四组件健康告警和 Web 终端低权限隔离均已完成首轮源代码实现；当前入口转为发布密钥轮换与撤销。
...
| 服务启动 | 默认 IP/HTTP、可选 HTTPS、双监听预绑定、systemd 自动应用、健康确认和失败恢复已完成 | Ubuntu systemd/断电恢复矩阵和无损监听切换 |
| 用户设置 | 当前密码确认、改密全会话失效、首次登录保护、TOTP 与设备会话管理已完成 | 多管理员角色、强制 MFA 策略和管理员重置 MFA |
| 系统监控与告警 | 分钟级历史、阈值状态机、四组件服务健康、静默/恢复、加密 Webhook、投递记录和前端告警中心已完成 | Linux 长时间运行、原生通知通道、证书/网站可用性规则和更多组件 |
...
| 软件管理 | Center/内置包、安装/升级/卸载/启停/重启/重载/结构化配置持久化任务、状态/版本探测、配置差异/快照/自动回滚、配置历史/手动恢复、四组件健康告警、进度、取消、恢复和统计已完成 | 真实 systemd/组件语法/历史恢复/健康告警矩阵和更多组件 |
| Web 终端 | 默认关闭；独立低权限用户、资源限制、主会话绑定、命令摘要审计和并发限额已完成源码实现 | Ubuntu/Rocky PTY、权限边界、吊销、并发和资源限制实机矩阵 |
...
| 面板访问 | 默认 IP/HTTP、独立可选 HTTPS、原子配置、systemd 自动应用/失败恢复、可信代理、安全头和前端设置已完成 | 真实 systemd/IP SAN/反向代理/IPv6/断电恢复矩阵 |
| 更新与回滚 | 签名、迁移预检、双版本切换、健康确认、失败/中断恢复及 Center 密钥轮换/撤销源码闭环 | 密钥轮换矩阵、Ubuntu 真实 systemd 故障矩阵和来源证明 |
| Docker | 本地测试部署已完成 | 继续补充镜像发布、升级与多架构流水线 |
| 备份迁移 | 数据库本地备份、网站整站备份及 Panel 配置/SQLite/实例身份/证书加密灾备已完成源码实现 | 数据库定时备份、多数据库、S3 和真实断电恢复矩阵 |
...
- Web 终端已完成独立低权限用户、capability 清理、资源限制、命令提交审计和并发控制源码实现；尚缺 Ubuntu/Rocky 实机权限逃逸、会话吊销和资源上限矩阵。
- 会话吊销、首次登录强制改密、TOTP、恢复码和前端设备管理已完成；实例凭据密钥已进入加密灾备包，生产仍需密钥轮换和恢复演练。
- 数据库连接和库用户密码已完成实例级加密和 API 脱敏；实例凭据密钥已随配置进入加密灾备包，仍需制定轮换运维流程。
- 审计日志已持久化并具备 HMAC 链、尾部截断检测、管理员查询/导出和保留清理；仍需外部不可变链头锚点防御整库快照回滚。
- 安装矩阵已进入 CI 配置，但尚未在当前未提交分支获得 GitHub Hosted Runner 的首次结果。
- 签名更新、备份、迁移预检、失败/中断恢复和 Center 密钥生命周期已完成源码闭环，但尚未取得密钥轮换及真实 systemd 升级/断电故障矩阵结果。
- 发布包已有 SHA-256、签名更新清单、SBOM 和许可证证据，尚缺来源证明、密钥轮换实机演练和变更日志自动校验。
...
Panel 配置与数据加密灾备已完成首轮源码实现。下一代码开发入口转为数据库定时备份计划、保留份数和远端目标；随后补齐网站访问/错误日志管理、文件分片上传和插件基础框架。远程验收入口为 Panel 离线恢复/断电回滚、密钥轮换、低权限终端权限边界、四组件服务控制、面板访问自动恢复、网站整站备份、签名更新/回滚、防火墙、公共 ACME/Nginx、Ubuntu 组件安装矩阵和 MySQL 8 数据库闭环矩阵。
...
7. 四组件状态、服务控制、结构化配置安全发布、历史恢复、服务健康告警、面板访问自动恢复和 Web 终端低权限隔离已完成首轮源码实现；下一步进入发布密钥轮换与撤销。
...
### 2026-07-30 Panel 更新签名密钥生命周期

状态：源码实现完成，按本轮要求未执行构建、测试或部署，等待验证后关闭任务。

完成：

- Center 新增独立的 Panel 更新签名密钥环，组件脚本包签名身份与 Panel
  版本签名身份不再强制共用一个长期在线密钥。
- 密钥支持 `staged`、`active`、`retired`、`revoked` 四种状态；只允许
  “预发布新密钥 → 激活新密钥并退役旧密钥 → 最后撤销旧密钥”的安全顺序，
  活动密钥不能直接撤销。
- Center 对密钥环本地状态签名并原子写入，私钥使用 `0600` 独立保存；
  密钥状态版本单调递增，同一版本公开文档保持稳定。
- 新增公开签名密钥状态接口和管理员查询、预发布、激活、撤销接口；全部变更
  进入现有签名审计链。
- Panel 在每次 Center 版本解析前先获取密钥状态，用当前已信任密钥校验
  交叉签名；拒绝版本回退、相同版本内容变化和历史密钥被直接删除。
- Panel 原子保存最后接受的密钥状态，动态加入预发布/活动/退役密钥，并立即
  排除已撤销密钥；未由已有可信密钥授权的新密钥不能自我建立信任。
- Center 管理后台新增“版本签名密钥”页面，展示状态版本、活动密钥、生命周期
  和撤销原因，并对激活、撤销提供强提示和确认。
- Panel 设置页新增签名 Key ID、Center 密钥状态版本、可信密钥数量和撤销数量，
  可直接判断当前版本清单使用的签名身份。
- 新增 Center 密钥环生命周期测试源码和 Panel 预发布、激活、撤销、回退拒绝
  测试源码；同时补充 Center API、管理后台和 Panel 发布运维文档。

待验证：

- 按用户本轮要求未运行 `gofmt` 之外的编译、单元测试、前端构建、集成测试
  或部署；任务在这些验证完成前不标记为“已完成”。
- 需要在至少两台 Panel 上执行“在线节点已同步、离线节点未同步”的轮换矩阵，
  验证过早撤销时的拒绝行为和离线节点恢复流程。
- 生产环境需要把 `data/keys/center-ed25519.key`、
  `data/panel-update-keys/` 与发布索引纳入同一恢复点，并制定双人审批、离线
  备份和紧急吊销流程。
- GitHub Artifact Attestation/SLSA 来源证明、外部不可变审计链头锚点仍未
  实现，作为下一源码开发入口。

### 2026-07-30 Panel 配置与数据加密灾备

状态：源码实现完成，按当前开发要求未执行构建、测试、运行调试或部署，等待后续统一验证。

完成：

- 新增加密 `.onebak` 灾备格式，使用 `scrypt` 派生独立密钥并通过
  XChaCha20-Poly1305 分块加密；密码限制为 12–256 字节，不写入任务表、
  操作审计或运行日志。
- 使用 SQLite `VACUUM INTO` 创建在线一致性快照；备份范围包含
  `config.yaml`、`myadmin.db`、实例 ID、Panel 更新可信密钥状态和可选
  Panel 证书目录，因此 JWT、TOTP、数据库凭据等实例加密数据能够在同一
  恢复点恢复。
- 备份清单记录文件类型、权限、大小和 SHA-256；创建时拒绝符号链接、
  特殊文件、超量文件和源文件读取期间变化，导入、下载和恢复前重新检查
  加密包摘要。
- 导入备份时先完整解密到受限临时目录，再严格检查清单、路径穿越、重复或
  额外文件、配置端口、JWT/凭据密钥、证书路径和 SQLite `quick_check`，
  校验失败的文件不会进入可用备份列表。
- 恢复由独立 `one-panel-restore.service` 接管；主 Panel 停止后原子替换
  配置、数据库、实例身份、更新信任和证书目录，再启动主服务执行 90 秒
  健康检查。
- 新增落盘恢复事务日志。切换失败、健康检查失败或提交状态无法持久化时，
  自动停止新实例、恢复旧快照并重启；服务器在替换过程中中断时，
  `one.service` 启动前会自动完成未结束事务回滚。
- 新增管理员 API，覆盖创建、导入、列表、下载、删除、预检、恢复和状态；
  所有修改、预检、下载操作均进入敏感操作审计，普通用户无法调用。
- 设置页新增“配置与数据备份”，支持证书开关、密码二次确认、`.onebak`
  导入、摘要与版本展示、下载、精确恢复确认、状态重连和永久删除确认。
  恢复旧数据库导致当前会话失效时，页面会引导管理员重新登录查看结果。
- 新增加密往返、密码错误、尾随数据拒绝和敏感路由分类测试源码。

待验证与后续：

- 当前仅完成源码格式化和差异检查；遵循本轮要求，没有执行 Go/前端测试、
  构建、运行调试或部署，因此本模块尚不能标记为生产验收完成。
- 后续需要在原生 systemd Linux 上执行成功恢复、错误密码、篡改包、磁盘
  不足、健康检查失败、替换中断电、提交中断和恢复后会话失效矩阵。
- `.onebak` 密码不由 Panel 托管；管理员遗失密码时无法解密。生产必须至少
  保存两份离线副本，并通过独立密码库管理密码。
- 下一开发入口为数据库定时备份计划、保留份数和 S3 远端目标。

### 2026-07-30 软件商城安装链路排查与修复

状态：核心链路已修复并部署到原生测试服务器，正在完成组件实装矩阵。

已完成：

- 修复开发版 Panel 把 `dev` 作为版本号发给 Center 的问题。现在所有请求
  都携带可比较的语义版本，Center 对非法主机信息返回明确的 400 错误，
  不再产生内部 500。
- Center 软件目录改为按已发布兼容包动态生成。没有脚本包的版本会自动禁用，
  推荐版本也会自动回退到实际可安装版本，避免商城展示“可安装”但解析失败。
- 18 个开发通道组件包重新生成并发布，50 个软件版本均通过 Center 解析；
  脚本兼容范围支持预发布 Panel 版本。
- 修复 OneinStack 安装目标已经健康、但上游脚本尾部因无关服务启动失败而把
  整个任务误判为失败的问题。现在以目标组件的版本和服务健康检查为准，并在
  日志中保留上游非零退出警告。
- 修复 Redis systemd 服务名、受 `umask 027` 影响的可执行权限、失败回滚后
  遗留软链接，以及权限修复后没有重新启动服务的问题。
- 安装、卸载、服务控制和配置发布任务现在记录实际解析到的脚本包版本与来源，
  任务事件增加 `package_resolved`，便于定位 Center、缓存和内置包差异。
- SQLite 连接统一启用 WAL、10 秒写入等待和受控连接池，避免审计事件与任务
  进度同时写入时出现 `SQLITE_BUSY` 并把任务永久留在队列。
- Redis 服务标识统一为实际的 `redis-server.service`；Center Redis 2.1.1
  增加配置读取、修订冲突校验、白名单参数发布、语法检查、失败恢复和服务重启。
- 点击“重启”不再弹出任务抽屉；任务仍在右上角计数并保留日志，完成后只显示
  “重启成功”，失败时显示明确错误。
- Node.js 22.12.0 已在 Ubuntu 24.04 amd64 原生服务器完成真实安装；任务数
  从 1 自动回到 0，商城卡片无需刷新即变为“卸载”。
- Redis 8.4.0 已完成安装、重启、配置读取、参数发布、参数恢复和卸载实测；
  卸载后程序目录、systemd 单元和软链接均清除，RDB 数据保留到
  `/data/redis`。Redis 2.1.1 会在重装时继续使用该独立数据目录。
- Panel 后端相关单元测试通过；Center 目录、解析和清单测试通过；生成的组件
  动作脚本全部通过 `bash -n`。

部署：

- Panel：`v0.1.0-test.23`，systemd 原生运行，HTTP 端口 `8089`。
- Center：原生 systemd 运行，HTTP 端口 `8189`。
- 部署前二进制与数据库保留在服务器手工备份目录，未使用 Docker。

后续实装矩阵：

- 完成 Redis 8.4.0 修订包的重复安装、服务重启和卸载验证。
- 按推荐版本依次覆盖 Web、PHP、数据库、Java/Tomcat 和工具组件。
- 旧版 PHP/MySQL/MariaDB/Percona 在 Ubuntu 24.04 上失败时保持开发通道
  禁用，不在未经验证前提升为 beta 或 stable。

### 2026-07-30 网站 Web 服务器识别与配置管理

状态：开发、自动化验证和 Ubuntu 原生测试环境部署已完成。

已完成：

- 网站页不再固定假设使用 Nginx，会根据实际运行进程、已安装组件和可执行
  文件识别 Nginx 或 OpenResty，并展示名称、版本、运行状态和站点配置目录。
- 网站创建、修改、证书发布和配置检查统一使用当前识别到的 Web 服务器；
  OpenResty 使用自身的二进制、前缀、主配置和 `vhost` 目录。
- 新增受权限控制的配置文件列表、读取和保存接口，只允许管理当前 Web
  服务器配置根目录内的 `.conf` 文件，并拒绝路径穿越和符号链接。
- 配置保存前检查修订版本，避免覆盖其他管理员刚刚提交的内容；保存时自动
  创建历史备份并使用原子替换。
- 每次保存后执行完整配置语法检查；失败会自动恢复旧文件。服务正在运行时
  自动平滑重载，重载失败同样恢复旧文件。
- 网站页新增 Web 服务器状态卡片和配置管理抽屉，可直接选择配置文件、编辑、
  刷新并保存生效，未保存内容切换或关闭前会提示确认。
- 当前测试服务器实测识别为 OpenResty `1.27.1.2`，主配置包含
  `include vhost/*.conf`，语法检查通过，Panel `one.service` 正常运行。

验证：

- 后端全量 `go test ./...` 通过，新增测试覆盖配置列表、读取、保存、备份、
  语法失败回滚、修订冲突、路径穿越和符号链接拒绝。
- 前端 `npm run typecheck` 与 `npm run build` 通过，生产前端包已同步到
  Panel 嵌入资源。
- Panel `v0.1.0-test.27` 已部署到原生测试服务器，HTTP 首页和新增路由鉴权
  检查正常；未对线上 OpenResty 配置内容执行破坏性编辑测试。

后续：

- 增加配置历史列表、差异对比和一键恢复。
- 增加新建、重命名和删除站点配置文件的受控操作。
- 在网站列表中展示每个站点最终归属的 Web 服务器和配置文件路径。

### 2026-07-30 全服务器文件管理、搜索与操作记录

状态：开发、全量自动化验证和 Ubuntu 原生测试服务器部署已完成。

已完成：

- 文件管理默认根目录由 `/data/` 调整为服务器 `/`，可直接浏览 `/etc`、
  `/usr`、`/var`、`/opt`、`/home` 等服务器路径；原有路径穿越和根目录
  删除保护继续生效。
- 修复根目录中的目录符号链接被误判为普通文件的问题，安全目标目录现在可以
  正常进入；越界符号链接仍由 `os.Root` 拒绝。
- 原“搜索文件/目录”只有空处理器且没有路由。现在新增真实的文件名搜索，
  支持当前目录或全服务器、文件/目录类型筛选、大小写不敏感匹配和结果定位。
- 全盘搜索最多返回 500 项、最多访问 20 万项、默认 12 秒超时和两个并发任务；
  从 `/` 搜索时跳过 `proc`、`sys`、`dev`、`run` 等动态虚拟文件系统。
- 修复普通文件点击“打开”无反应，新增文本文件编辑抽屉，展示路径、大小、
  权限、所有者和 MIME 类型；二进制文件和超过在线编辑上限的文件会被拒绝。
- 文本保存增加 SHA-256 修订校验，文件被其他进程修改时返回冲突，不会静默
  覆盖新内容。
- 文件页面新增“操作记录”，记录读取、保存、新建、上传、下载、URL 下载、
  权限修改、移入回收站、恢复、彻底删除和清空回收站。
- 每条文件操作记录包含具体路径、账号、来源 IP、结果、说明和时间，并进入
  现有 HMAC 防篡改审计链；文件页面只需文件读取权限即可查看本模块记录。
- 根目录容量查询不再递归扫描整台服务器；未配置目录配额时直接读取文件系统
  容量，避免进入虚拟文件系统或因文件数量过大拖慢页面。

验证与部署：

- 后端 `go test ./...` 全量通过；新增测试覆盖名称搜索、类型筛选、结果上限、
  路径穿越、符号链接不跟随、修订冲突、二进制拒绝和具体操作审计记录。
- 前端 `npm run typecheck` 与 `npm run build` 通过，生产 ZIP 已同步至 Panel。
- Panel `v0.1.0-test.28` 已部署到 `192.168.1.6:8089`；`one.service` 为
  active，首页返回 HTTP 200，搜索和操作记录路由均已受登录鉴权保护。
- 测试服务器旧配置 `defaultpath: /data/` 已备份并迁移为
  `defaultpath: /`；业务文件未被修改。

后续：

- 增加复制、移动、重命名、压缩和解压任务。
- 为跨文件系统删除实现独立回收站复制任务和可视化进度。
- 为系统关键文件保存增加自动快照、内容差异和一键恢复。

### 2026-07-30 文件列表与文件操作完善

状态：开发、自动化验证和 Ubuntu 原生测试环境部署已完成。

已完成：

- 文件列表把“权限”和“用户”合并为“权限 / 所有者”双行信息，不再单独
  展示用户组列；权限和所有者仍可分别点击修改。
- 每行增加复制、剪切、重命名、权限、压缩、删除和“更多”操作；更多菜单
  包含打开、在新窗口打开、收藏、外链分享、下载和属性，没有加入木马扫描。
- 增加文件剪贴板和“粘贴到当前目录”，复制、移动和重命名均不会覆盖同名
  文件；剪切成功后自动清空剪贴板。
- 新增普通文件和目录的递归复制、同文件系统原子移动、跨文件系统安全
  复制后删除，以及独立重命名接口。
- 新增 `.tar.gz` 压缩包创建；压缩和复制都拒绝符号链接、设备文件等特殊
  类型，不会跟随链接访问管理根目录之外的文件。
- 新增文件属性窗口，展示路径、类型、权限/所有者、大小、MIME 和修改时间，
  不展示用户组。
- 新增浏览器本地收藏夹和新窗口打开，收藏最多保存 100 项。
- 新增普通文件外链分享，有效期可设为 1–168 小时；数据库只保存随机令牌
  的 SHA-256 哈希，令牌不会写入 HTTP 路径日志。分享文件被修改、删除、
  过期或取消分享后，下载链接立即失效。
- 外链分享记录支持查看和取消；复制、移动、重命名、压缩、分享创建、分享
  下载和取消分享均进入现有文件操作审计链。

验证与部署：

- 文件管理和路由定向测试通过；新增测试覆盖递归复制、同名拒绝、移动、
  重命名、压缩包内容、符号链接拒绝、属性读取、令牌哈希和文件变更失效。
- 后端全量测试中，所有业务测试通过；两个需要监听本地随机端口的测试在
  沙箱内被系统拒绝后，已在允许本地监听的环境中重跑通过。
- 前端 `npm run typecheck` 和 `npm run build` 通过，构建 ZIP 已同步到
  Panel 嵌入资源。
- Panel `v0.1.0-test.29` 已原生部署到 `192.168.1.6:8089`；服务为
  active，首页 HTTP 200，公开分享无令牌返回 404，复制接口未登录返回 401。
- 旧版二进制已备份到
  `/usr/local/one/backups/manual-release-v0.1.0-test.29/one-v0.1.0-test.28`。

后续：

- 增加压缩包在线解压、压缩/复制大任务的后台进度和取消能力。
- 增加收藏夹的账号级云端同步，以及外链分享记录管理页面。
- 为 `/etc` 等系统关键目录的覆盖式保存增加自动快照和差异恢复。

### 2026-07-31 文件操作布局与图片预览

状态：开发、自动化验证和 Ubuntu 原生测试环境部署已完成。

已完成：

- 重新整理文件列表操作区，只保留打开或预览、复制、剪切、重命名和
  “更多”作为行内主操作；权限、压缩、属性和删除收纳到分组菜单，减少
  横向拥挤并统一按钮间距、圆角和悬停反馈。
- 图片文件使用独立图标显示，点击文件名或“预览”可打开图片预览窗口；
  支持等比适配、全屏查看、缩放和下载原图。
- 支持 JPEG、PNG、GIF、WebP、BMP、ICO 和 AVIF，单张在线预览上限为
  30 MB；SVG 因可能包含主动脚本内容，继续按普通文件处理且不允许预览。
- 图片预览拒绝符号链接、目录和设备文件，不会通过预览接口绕过文件读取
  权限或既有文件管理根目录限制。
- 预览地址不再直接携带服务器文件路径。前端先申请有效期 5 分钟的一次
  会话绑定随机凭证，再使用随机凭证读取图片，避免真实路径进入访问日志；
  其他账号或其他会话不能复用该凭证。
- 预览读取进入现有文件操作审计链，响应增加 `nosniff`、沙箱 CSP 和私有
  缓存策略。

验证与部署：

- 新增后端测试覆盖正常图片、SVG 拒绝、超大文件拒绝、跨会话凭证拒绝和
  安全响应头；文件管理和路由测试通过。
- 前端 `npm run typecheck` 和 `npm run build` 通过，构建 ZIP 已同步到
  Panel 嵌入资源。
- Panel `v0.1.0-test.31` 已原生部署到 `192.168.1.6:8089`；服务为
  active，首页 HTTP 200，预览凭证和图片读取接口均受登录鉴权保护。
- 旧版二进制已备份到
  `/usr/local/one/backups/manual-release-v0.1.0-test.31/one-v0.1.0-test.30`。
- Codex 内置浏览器沙箱无法路由到该内网地址，因此未在该浏览器内完成
  截图验收；自动化测试、生产构建、服务状态和 HTTP 健康检查均已通过。

后续：

- 增加视频、音频和 PDF 的安全在线预览。
- 为图片预览增加旋转、原始尺寸切换和多图前后浏览。
- 增加列表显示密度切换及账号级偏好保存。

### 2026-07-31 全局主题颜色切换

状态：开发、自动化验证、生产构建和 Ubuntu 原生测试环境部署已完成。

已完成：

- 设置页面新增“外观与主题”，无需重新加载即可切换浅色和深色模式。
- 提供活力橙、科技蓝、翡翠绿、极光紫、珊瑚红和深海青六套预设主题色，
  并支持通过颜色选择器使用自定义颜色。
- 主题引擎会根据所选颜色自动生成按钮悬停、浅色背景、边框、焦点光环等
  完整色阶，同时更新 Element Plus 全局颜色变量。
- 导航选中态、按钮、链接、表单焦点、分页、抽屉、任务进度、文件管理、
  登录页面、首次登录页面、404 页面和终端光标均接入全局主题色。
- 成功、警告、错误等业务状态色保持独立，不会因主题色切换而失去语义。
- 界面模式和主题色保存到当前浏览器本地存储，退出登录或刷新页面后继续
  生效；“恢复默认”可一键回到浅色和活力橙。
- 主题应用不再直接覆盖根节点全部 CSS 类名，避免清除其他运行时样式类。

验证与部署：

- 新增 3 项主题工具单元测试，覆盖颜色格式标准化、非法颜色回退和色阶混合。
- 前端 `npm run typecheck`、主题单元测试和 `npm run build` 全部通过，
  生产 ZIP 已同步到 Panel 嵌入资源。
- Panel `v0.1.0-test.32` 已原生部署到 `192.168.1.6:8089`；上传文件
  SHA-256 校验通过，`one.service` 为 active，首页返回 HTTP 200。
- 旧版二进制已备份到
  `/usr/local/one/backups/manual-release-v0.1.0-test.32/one-v0.1.0-test.31`。

后续：

- 将主题偏好从浏览器级扩展为账号级同步，用户在不同设备登录时自动恢复。
- 增加品牌 Logo、登录页背景和紧凑度等更多外观配置。
- 增加高对比度和跟随操作系统模式。

### 2026-07-31 安全防火墙规则中心

状态：开发、自动化验证、生产构建和 Ubuntu 原生测试环境部署已完成。

已完成：

- 安全页增加防火墙总开关、禁止 Ping 和过期规则清理入口；继续保留
  firewalld 自动安装、修复、面板端口保护和防火墙持久化状态提示。
- 规则按端口、IP、端口转发、地区和恶意 IP 自动封禁五类展示，并实时显示
  每类规则数量。
- IP 规则支持 IPv4/CIDR、放行或拒绝、启用或停用、备注、归属类型、
  创建时间和过期时间；拒绝阻断全部 IPv4 入站流量，防止误操作导致失联。
- 端口和 IP 规则支持按 IP/备注搜索、分页、选择、批量启用、批量停用、
  批量删除，以及 JSON 导入和导出。导入中途失败会回滚本次已创建规则。
- 地区规则使用“地区名称 + 明确 CIDR 网段”管理，不依赖不稳定的在线
  GeoIP 接口，也不会把地区名称直接转换成未经校验的防火墙命令。
- 增加 firewalld 端口转发的创建、编辑、启停和删除；源端口不能占用当前
  面板管理端口，目标地址必须是有效 IPv4。
- 增加 SSH 恶意 IP 自动封禁：默认关闭，管理员可设置失败次数、统计周期
  和封禁时长；开启后每分钟读取 SSH 失败登录日志，忽略本机和内网地址，
  自动创建带过期时间的拒绝规则。
- 后台维护任务每分钟清理已过期的临时规则；页面“清理缓存”可立即执行同一
  清理流程，不会刷新全部防火墙或中断现有连接。
- 所有新增写接口进入权限控制和敏感操作审计范围；数据库迁移会把历史规则
  自动归类为端口规则。

验证：

- 新增测试覆盖全协议 IP 规则、全 IPv4 拒绝保护、禁用规则延迟应用、
  过期规则清理和 firewalld 端口转发生命周期。
- 后端 `go test ./...` 全量通过。
- 前端 `npm run typecheck`、8 项 Vitest 测试和 `npm run buildNocnd`
  全部通过。
- 生产 Web ZIP 已同步到 Panel 嵌入资源，SHA-256 为
  `de89f6046c2ff69e83916ece7fc9b7d2cbdee03068f8f57239207f088eb386f6`。
- Panel `v0.1.0-test.33` 已原生部署到 `192.168.1.6:8089`；`one.service`
  为 active，首页返回 HTTP 200，firewalld 为 running。
- 数据库迁移已在测试机完成，历史 `iptables_rules` 增加规则类型、归属地和
  过期时间字段，并创建端口转发、自动封禁配置表。
- 测试机二进制 SHA-256 为
  `bf74b26bbbbf41894f5578377b15c8d7a696bbc2645702ca338e43c85402ea86`；
  旧版已备份到
  `/usr/local/one/backups/manual-release-v0.1.0-test.33/one-v0.1.0-test.32`。

后续：

- 由管理员在页面验证 firewalld IP 规则、端口转发、规则过期和 SSH 自动
  封禁的交互流程；自动封禁继续保持默认关闭。
- 接入可离线更新的 GeoIP 数据库，为公网 IP 提供国家、省市和 ASN 信息。
- 增加规则命中计数、命中日志和自动封禁白名单。

### 2026-07-31 文件操作按钮悬停显示

状态：开发、构建和 Ubuntu 原生测试环境部署已完成。

已完成：

- 桌面端文件列表的操作列默认保持空白，鼠标移动到当前文件行后才显示打开、
  复制、剪切、重命名和更多操作，减少大列表中的视觉干扰。
- 操作按钮增加轻量淡入和位移动画，离开当前行后自动隐藏。
- 当前行通过键盘获得焦点时同样显示操作按钮；触屏设备没有稳定 hover 能力，
  因此继续常驻显示，避免手机和平板无法操作文件。

验证与部署：

- 前端 `npm run typecheck` 和 `npm run buildNocnd` 通过。
- WebUI 版本更新为 `1.0.1`，资源文件使用新哈希，生产 ZIP SHA-256 为
  `bdb0f5c048f54edd06ac9daceb546d90c25faf8a4cf3e76702c311664ef96410`。
- Panel `v0.1.0-test.34` 已部署到 `192.168.1.6:8089`；`one.service`
  为 active，首页返回 HTTP 200。
- 旧版本已备份到
  `/usr/local/one/backups/manual-release-v0.1.0-test.34/one-v0.1.0-test.33`。

### 2026-07-31 数据库环境引导与 Redis 本机连接自愈

状态：开发、自动化验证、生产构建和 Ubuntu 原生测试环境部署已完成。

已完成：

- MySQL、Redis 管理页在没有本机环境和远程连接时显示明确的环境缺失提示，
  可直接跳转到对应类型的远程数据库添加页或软件商城安装页。
- 软件商城支持通过 `component=mysql`、`component=redis` 查询参数自动筛选目标
  组件，从数据库空状态跳转后不再需要手动搜索。
- Redis 安装任务成功后会立即验证并登记本机连接，默认使用
  `127.0.0.1:6379` 和 Redis ACL 的 `default` 用户。
- 升级前已经安装 Redis、但历史版本没有保存连接记录的服务器，会在 Panel
  启动或读取 Redis 连接列表时自动补建记录；连接测试失败时不会写入错误记录。
- Redis 客户端正式支持 ACL 用户名；当低权限 Redis 禁止 `CONFIG GET` 时，
  使用兼容的 16 个逻辑库回退，不再因此让整个管理页无法加载。
- Redis 已存在连接但当前逻辑库没有 Key 时，页面显示正常的空库提示，不再
  错误提示用户安装环境。

验证与部署：

- 后端 `go test ./...` 全量通过；新增单元测试覆盖已安装 Redis 缺少连接记录时
  的自动验证、补建和默认参数。
- 前端 `npm run typecheck`、8 项 Vitest 测试和 `npm run buildNocnd` 全部通过。
- WebUI 更新为 `1.0.2`，生产 ZIP SHA-256 为
  `289bf4cfbdfdaf70567712234830dffd4b399a66f18163a72f1c28d6017bcb65`。
- Panel `v0.1.0-test.35` 已原生部署到 `192.168.1.6:8089`；最终二进制
  SHA-256 为
  `4e315ba1f0a5d8950919038b62b9712339ce9fbe0fae2a48bee7c49e8892df2b`，
  `one.service` 为 active，健康检查返回 HTTP 200。
- 测试机 Redis 8.4.0 正在 `127.0.0.1:6379` 运行，DB0 保留 1 个 Key；新版
  启动后已自动创建“本机 Redis（面板自动管理）”连接记录，未修改 Redis 数据。
- 旧版本已备份到
  `/usr/local/one/backups/manual-release-v0.1.0-test.35/one-v0.1.0-test.34`。

后续：

- 在浏览器中分别验收 MySQL/Redis 未安装提示、远程数据库跳转和软件商城筛选。
- 增加 Redis Key 新建、查看值、编辑值、删除和过期时间管理。

### 2026-07-31 数据库环境提示与 Redis DB 参数修复

状态：问题已复现、修复、构建并部署到 Ubuntu 原生测试环境。

已完成：

- 根据测试机请求日志定位 Redis Key 列表返回 400 的原因：前端把 DB 编号
  `"0"` 作为字符串提交，而后端接口要求整数。现在切换逻辑库时统一转换为
  非负整数，并把分页重置到第一页。
- Redis 逻辑库标签与请求参数统一使用数字类型，避免标签已选中但请求仍使用
  字符串的状态不一致。
- MySQL 环境提示不再单纯依据连接记录数量判断。软件状态为未安装且数据库中
  只剩历史“面板自动管理”记录时，会把该记录视为失效本机连接并显示安装提示；
  已配置的真实远程连接不受影响。
- 测试机确认 MySQL 软件记录均为未安装，但存在一条历史本机连接记录；Redis
  DB0 中的 `codex:preserve-test` Key 保持存在。

验证与部署：

- 前端 `npm run typecheck` 与 `npm run buildNocnd` 通过。
- Panel `v0.1.0-test.36`、WebUI `1.0.3` 已部署到
  `192.168.1.6:8089`；服务为 active，健康检查返回 HTTP 200。
- WebUI ZIP SHA-256 为
  `d06afd8dc87f6a22dca847cfa20d40dc0a46ae5d418fe4c0259a9f7d959b6ad3`，
  最终二进制 SHA-256 为
  `f8c3d8cf2202fe34220dd7de1adce7b4c2c522a5a9810396d91f8daa3015d1d1`。
- 旧版本已备份到
  `/usr/local/one/backups/manual-release-v0.1.0-test.36/one-v0.1.0-test.35`。

### 2026-07-31 MySQL 安装链路修复与原生环境验收

状态：安装逻辑已修复，Center 新包已发布，Ubuntu 原生测试机全链路安装通过。

已完成：

- MySQL 安装统一使用经过校验的独立 OneinStack 风格脚本包，不再调用会受旧镜像
  `.md5` 地址影响的通用上游适配脚本；development 通道仅保留已验证的 MySQL 8.0。
- Center 发布 MySQL `2.0.4` development 包和 `1.0.7` stable 包；安装包固定为
  MySQL `8.0.45` 官方 generic binary，并校验 SHA-256。
- 修正 MySQL systemd 服务名，统一使用真实存在的 `mysql.service`。
- 安装任务会复用面板保存的本机 MySQL root 凭据；首次安装才生成随机密码，已安装
  但凭据丢失时明确报错，不再以空密码继续执行。
- 已存在的面板 MySQL 连接会在安装完成后重新解密并执行连接测试，避免数据库里有
  连接记录但实际凭据不可用。
- 配置脚本在收到 root 密码时始终执行 TCP 回环登录验证；已有数据目录也必须通过
  凭据验证后任务才可成功。
- 回滚脚本增加事务标记。下载、预检等尚未建立回滚点的失败不会再误执行卸载逻辑，
  避免删除仍在运行的 MySQL 程序目录。

验证与部署：

- Panel `v0.1.0-test.37` 已部署到 `192.168.1.6:8089`；Center 为该主机解析到
  MySQL `2.0.4` remote 包，签名摘要校验一致。
- 真实安装任务 `822eef64-d98d-4a7e-ab0a-1998dd1f2732` 成功，37 个任务事件完整，
  最终状态 `succeeded`、进度 100%，无错误码和回滚残留。
- 测试机 `mysql.service` 为 enabled/active；运行进程来自
  `/usr/local/mysql/bin/mysqld`，版本为 MySQL `8.0.45`，仅监听
  `127.0.0.1:3306`。
- 软件商城数据库状态已更新为 MySQL 8.0 已安装；安装后回环连接和版本健康检查通过，
  原有数据目录与面板管理凭据得到保留并继续可用。
- Panel 全量 Go 测试通过；需要监听本地测试端口的 `panelupdate` 和 `server` 包已在
  非沙箱环境补跑通过。Center 的 shell 语法、组件包、manifest 和 catalog 测试通过。

### 2026-08-01 软件商城原生安装审计

状态：18 个 development 组件均能从 Center 解析；推荐组件已完成原生安装审计、
修复或明确的环境兼容阻断。

已完成：

- 在 Ubuntu 24.04 原生测试机逐项执行软件商城安装，Web 服务、PHP、Java、Tomcat、
  PostgreSQL、Node.js、FTP、phpMyAdmin、Memcached、MySQL 和 Redis 均完成运行验证。
- 修复 Nginx、Apache、Java、Tomcat、PHP、PostgreSQL、MongoDB、MariaDB 的上游下载、
  版本适配、服务启动或卸载问题，离线源包统一保存并生成 SHA-256 清单。
- Panel 增加 Web 服务和 MySQL 兼容数据库互斥保护；MySQL 已安装时 MariaDB、Percona
  在入队前直接拒绝，不再运行脚本后失败。
- MongoDB 8.0 增加 AVX 预检。当前测试机不提供 AVX，因此明确标记为环境不兼容，
  不再下载完成后以非法指令崩溃。
- firewalld `1.0.0` 正式组件已加入 Center stable 通道；18 个 development 组件解析
  接口全部返回 HTTP 200。
- Panel `v0.1.0-test.39` 已部署到 `192.168.1.6:8089`，服务为 active。

验证：

- Panel `go test ./internal/services/... ./app ./router/handler/software/...` 通过。
- Center 19 个 development/stable 组件包全部可构建，Shell 语法检查通过。
- Center manifest、software catalog、HTTP API 测试通过。
- 完整审计矩阵见 [`SOFTWARE_STORE_INSTALL_AUDIT.md`](./SOFTWARE_STORE_INSTALL_AUDIT.md)。

后续：

- 使用无现存 MySQL 数据的干净主机补测 MariaDB 10.11 与 Percona 8.0。
- 使用提供 AVX 的 amd64 主机补测 MongoDB 8.0.17。
- 分批补齐 PHP 5.3—8.5 全版本编译矩阵。

### 2026-08-01 网站根目录跳转文件管理

状态：已完成。

已完成：

- 网站管理列表的“根目录”改为带文件夹图标的可点击路径，长路径保持省略显示并可通过
  悬浮标题查看完整内容。
- 点击根目录后通过 `/file?path=<网站根目录>` 跳转到文件管理，文件页面复用现有目录
  定位逻辑并直接加载对应服务器目录。
- 没有根目录的站点显示占位符，不生成无效跳转；文件管理权限仍由全局路由权限矩阵
  统一校验。
- 重新构建 `Oneinstack-Panel-Web`，并把生成的 `app-1.0.0.zip` 同步到 Panel
  `webui/app.zip`。

验证：

- 前端 `vue-tsc --noEmit` 通过。
- 前端 Vitest 3 个测试文件、8 个测试全部通过。
- 前端生产构建通过；Panel `go test ./webui ./cmd` 通过。
- Panel `v0.1.0-test.40` 已重新构建并部署到 `192.168.1.6:8089`；
  `one.service` 为 active/enabled，live、ready、数据库和 WebUI 健康检查全部通过。
- 线上首页 SHA-256 与本地嵌入包一致，线上网站页面分块已确认包含根目录跳转代码；
  服务器保留升级前 `test.39` 二进制用于紧急回滚。

### 2026-08-01 网站默认首页与防火墙端口联动

状态：代码开发、自动化测试和 Ubuntu 测试环境部署已完成。

已完成：

- 静态网站和 PHP 网站创建时，如果根目录中没有 `index.html`，自动生成响应式
  OneinStack 默认首页；已有首页不会被覆盖，反向代理站点不创建无意义的本地目录。
- 新建网站目录权限调整为 `0755`，默认首页权限为 `0644`，保证 Nginx/OpenResty
  工作进程能够读取，同时不授予页面写权限。
- 网站配置解析结果保留实际监听端口，支持默认 80 以及域名中显式填写的自定义端口。
- 主机防火墙已经启用时，网站创建流程自动新增全 IPv4 入站 TCP 放行规则，规则备注
  使用规范化网站名；防火墙未安装或未启用时不修改主机规则。
- 自动放行前会识别系统中真实生效且已覆盖目标端口的规则，多个网站共用 80/443 时
  复用已有规则，避免重复规则不断累积。
- 网站数据库写入或 Web 配置发布失败时，会补偿删除本次新增的默认首页、空目录和
  防火墙规则，保持数据库、配置文件、站点目录与主机防火墙状态一致。

验证：

- `go test ./internal/services/safe ./internal/services/website` 通过。
- 覆盖防火墙关闭不操作、开启后自动放行、自定义端口、规则复用、备注写入、默认首页
  生成和网站发布失败回滚等路径。
- Panel 全量 Go 测试通过；其中需要监听本机临时端口的 `panelupdate` 和 `server`
  测试包已在非沙箱环境补跑通过。
- Panel `v0.1.0-test.41` 已部署到 `192.168.1.6:8089`；`one.service` 为
  active/enabled，live、ready、数据库和 WebUI 检查均通过，最近五分钟无 warning 日志。

后续：

- 在界面分别创建默认 80 和自定义端口网站，完成 firewalld 规则备注、默认首页浏览器
  访问以及删除网站后的端口规则生命周期验收。

### 2026-08-01 数据库 phpMyAdmin 快捷安装与管理

状态：前端开发、测试、嵌入包构建和 Ubuntu 测试环境部署已完成。

已完成：

- MySQL 数据库页增加 phpMyAdmin 快捷管理卡片，统一展示未安装、安装中和已安装状态，
  不再保留不可用的注释占位按钮。
- “快捷安装”直接调用 Panel 的可信软件任务接口，继续使用 Center 控制的组件版本、签名
  校验、安装阶段、实时日志、取消和失败回滚能力。
- 安装前检查 PHP 和 Web 服务依赖；缺少依赖时明确提示，并可直接跳转到软件商城中对应
  的 PHP 或 Nginx 页面。
- 根据已安装 PHP 主版本自动选择兼容的 phpMyAdmin 版本：PHP 5.3–7.1 使用
  `4.4.15.10`，PHP 7.2 及以上优先使用 `5.2.3`。
- phpMyAdmin 已安装后可从卡片直接打开；数据库列表每一行增加“快捷管理”，打开时只携带
  数据库名，不在 URL 中传递数据库用户名或密码。
- 软件商城的组件定位参数扩展到 `php`、`phpmyadmin` 和 `nginx`，依赖跳转后能直接筛选
  对应组件。
- 重新生成正式 WebUI 包并同步到 `webui/app.zip`，SHA-256 为
  `c9d4ce6deb84cc72234f7dd91a2b91f2378791969556a938209edf19335bad7d`。

验证：

- 前端 `vue-tsc --noEmit` 通过。
- 前端 Vitest 3 个测试文件、8 个测试全部通过。
- 前端 `npm run buildNocnd` 通过；Panel `go test ./webui ./cmd` 通过。
- 测试机数据库页静态分块与本地发布包 SHA-256 一致，并确认包含 phpMyAdmin 依赖提示、
  安装任务、快捷打开和按库跳转代码。

后续：

- 在浏览器验收 PHP 8.3 + phpMyAdmin 5.2.3 的快捷安装、任务进度、登录和按库跳转流程。
- 测试机升级前 `test.40` 二进制保存在
  `/usr/local/one/backups/manual-release-v0.1.0-test.41/one-v0.1.0-test.40`。

### 2026-08-01 数据库列表操作栏整理

状态：前端开发、自动化测试、嵌入包构建和 Ubuntu 测试环境部署已完成。

已完成：

- 数据库列表只保留“快捷管理、查看账号、修改密码”三个常用操作。
- 将“立即备份、备份管理、删除数据库”统一收进“更多”下拉菜单，删除操作继续使用
  危险色，并保留原有数据库名二次确认流程。
- 操作区域改为单行弹性布局，取消按钮默认外边距，并为操作列设置 320px 最小宽度，
  正常桌面分辨率下不再换成两排。
- 重新生成正式 WebUI 包并同步到 `webui/app.zip`，SHA-256 为
  `5783085c04aad1ceeef438c95df32a0e6338928fb0d9fcc8027e2d6b2f480575`。
...
- 前端 Vitest 3 个测试文件、8 个测试全部通过。
- 前端 `npm run buildNocnd` 通过；Panel `go test ./webui ./cmd` 通过。
- Panel `v0.1.0-test.42` 已部署到 `192.168.1.6:8089`；服务为 active/enabled，
  live、ready、数据库与 WebUI 健康检查均通过，最近五分钟无 warning/error 日志。
- 测试机数据库页面静态分块已确认包含“立即备份、备份管理、删除数据库、更多”；升级前
  `test.41` 保存在
  `/usr/local/one/backups/manual-release-v0.1.0-test.42/one-v0.1.0-test.41`。

### 2026-08-01 单站点设置工作台与网站生命周期

状态：前后端开发、自动化测试、正式前端构建、嵌入包同步和 Ubuntu 测试环境部署已完成。

已完成：

- 网站列表增加独立启停开关、今日响应流量、今日请求数和到期时间；关闭网站会安全移除
  对应虚拟主机配置，重新开启时经过配置语法检查并平滑重载。
- 增加每分钟网站生命周期调度：增量读取各站点访问日志并按天统计请求和流量；网站到期
  后自动关闭并记录关闭原因，已到期网站不能被直接重新开启。
- 新增宽屏单站点设置工作台，包含域名管理、子目录绑定、网站目录、访问限制、流量限制、
  伪静态、默认文档、配置文件、SSL、PHP、重定向、反向代理、防盗链、防篡改、网站安全、
  网站日志、网站告警和其他设置。
- 目录、IP/CIDR、代理地址、重定向地址、PHP-FPM 地址和伪静态指令均由后端白名单校验，
  发布前执行 Nginx/OpenResty 语法检查，失败时恢复原配置并保持网站继续可用。
- 原始配置文件提供受管读取、编辑、校验和发布；结构化设置会生成可信配置，ACME 验证路径
  保留最高优先级，避免访问限制、防盗链或重定向破坏证书续签。
- 防篡改可定期检查站点虚拟主机配置，发现非面板修改后恢复可信版本；网站流量超过设置的
  日阈值时写入运行告警日志，同一站点同一天只提醒一次。
- 网站备份归档同步保存全部结构化设置，恢复网站时同时恢复设置并重新生成可信配置；旧版
  不含设置的备份继续兼容。
- 网站删除同时清理设置、每日流量和日志游标记录，避免删除后残留无归属运行数据。

验证：

- Panel `go test ./...` 全量通过，覆盖启停、到期关闭、日志增量统计不重复、配置渲染与
  安全校验、防篡改恢复、设置发布，以及备份/删除后的完整设置恢复。
- 前端 `vue-tsc --noEmit` 通过；Vitest 3 个测试文件、8 个测试全部通过。
- 前端 `npm run buildNocnd` 通过，正式 WebUI 已同步到 `webui/app.zip`；Panel
  `go test ./webui ./cmd` 通过。
- 新 `webui/app.zip` SHA-256 为
  `88b5607f081eff3871f256d1df2a3b36ce8b6389ff0cd1da576231d912f01b5a`。
- Panel `v0.1.0-test.43` 已原生部署到 `192.168.1.6:8089`；服务为
  active/enabled，服务器本机与开发机访问 live、ready、首页均正常，数据库和 WebUI
  就绪检查通过。
- 测试机二进制 SHA-256 为
  `84b1b231df5a04ad0932d5b1038c26db069e25f6a2083f020f105ea8480a3752`；启动日志未发现
  panic、fatal、failed 或 error。
- 部署前 `test.42` 二进制、配置和 SQLite 数据库已保存到
  `/usr/local/one/backups/manual-release-v0.1.0-test.43`。

后续验收：

- 在 Ubuntu 测试机分别使用 Nginx 与 OpenResty 验收站点启停、配置发布、HTTP/HTTPS、
  PHP-FPM、代理、重定向、日志统计、到期自动关闭和异常配置回滚。
- 长时间运行验证访问日志切割、Panel 重启、配置被外部修改和多站点并发发布场景。

### 2026-08-01 网站配置自动恢复与设置弹窗收敛

状态：问题定位、前后端修复、自动化测试、正式构建和 Ubuntu 测试环境部署已完成。

已完成：

- 测试机确认站点 `www.spdzy.com` 在数据库中为启用状态，但对应
  `/usr/local/nginx/conf/vhost/www.spdzy.com.conf` 运行配置已丢失；旧接口只读取文件，
  因此持续返回“读取网站配置失败”。
- 网站配置读取改为使用与网站发布器同一次探测得到的 Nginx/OpenResty 配置管理器，避免
  运行中重复探测造成发布目录与读取目录不一致。
- 启用站点的配置文件在面板外被删除时，读取接口会根据数据库中的站点、TLS 和结构化设置
  重新生成可信配置；只有通过 Web 服务器语法检查和平滑重载后才返回编辑器。
- 增加配置目录归属校验和明确错误信息；停用站点没有运行配置时不再返回含糊的文件错误。
- 网站设置由接近全屏的右侧抽屉改为居中的中等尺寸弹窗，桌面端最大约 `980 x 720`，
  左侧菜单与右侧内容独立滚动，移动端保留安全边距。
- 配置文件页面增加内联错误详情、再次读取按钮、未加载占位状态；配置未成功读取前禁止
  发布，避免以空内容覆盖配置。

验证：

- 新增“启用站点配置丢失后自动重建”的回归测试；`go test ./...` 全量通过。
- 前端 `vue-tsc --noEmit` 和 `npm run buildNocnd` 通过；正式 WebUI 已同步到
  `webui/app.zip`，SHA-256 为
  `372d51d8909d92101265913f886e017d680bf224e187daefbaa7a0817a5cf828`。
- Panel `v0.1.0-test.44` 已部署到 `192.168.1.6:8089`；服务为 active/enabled，
  live、ready、首页、数据库和 WebUI 健康检查通过，开发机访问返回 HTTP 200。
- 测试机已确认新弹窗 CSS 和“再次读取”前端代码生效；部署后二进制 SHA-256 为
  `0cf8ba1f653575572f95cad7d53d6f5f0bf40a7809154bd0bb2c70ac8b02ef50`。
- 部署前 `test.43` 二进制、配置和 SQLite 数据库已保存到
  `/usr/local/one/backups/manual-release-v0.1.0-test.44`。

### 2026-08-02 软件商城 PNG 图标

状态：19 个在线品牌图标已替换为内置 PNG，Center 迁移和签名逻辑已完成。

已完成：

- 为当前商城 19 个产品补齐独立 PNG 图标，覆盖 Nginx、Tengine、OpenResty、Caddy、
  Apache、PHP、OpenJDK、Tomcat、MySQL、MariaDB、Percona、PostgreSQL、MongoDB、
  Node.js、Pure-FTPd、phpMyAdmin、Memcached、Redis 和 firewalld。
- 常用组件图标来自 Dashboard Icons/selfh.st 的 PNG 资源；Tengine、OpenResty、
  Pure-FTPd 和 firewalld 使用项目官网或官方仓库的位图资源，Percona 使用透明 PNG
  品牌资源。所有文件均已检查为真实 PNG，不再把 SVG 作为商城资源。
- 图标以内置 PNG Data URI 随 Center 签名目录下发，不依赖第三方图片地址，Panel 离线或
  外网不可用时仍能正常显示。
- Center 启动时会为旧版目录中缺失图标或仍为 SVG Data URI 的内置产品自动替换 PNG、
  重新签名并持久化；管理员设置的普通 URL/PNG 图标保持不变。
- 新增或重新保存内置产品时自动使用 `data:image/png;base64` 默认图标。
- 测试 Center 已通过管理 API 发布全部 PNG，签名目录修订号为
  `dd06244d6000627cc3ee3a19190ae6eeb85f319560814df09764070396f32ee2`。

验证：

- 19 个资源已通过文件类型检查和联系表预览检查，全部为 PNG。
- 默认目录和旧 SVG 签名目录的迁移测试均检查 `data:image/png;base64` 前缀。
- Center 管理目录和公共目录均确认 `19/19` 个产品为 PNG、SVG 数量为 `0`，签名有效。
- 测试 Panel 重启同步后，50 条可见软件版本全部为 PNG、SVG 数量为 `0`，同步错误为空；
  `one.service` 和 `oneinstack-center.service` 均为 active，Panel 首页返回 HTTP 200。

### 2026-08-07 Web 终端切换为 Root 模式

状态：后端、前端、安装器、自动化验证和 Ubuntu 测试环境部署已完成。

已完成：

- 取消 `one-terminal` 低权限隔离用户设计，Web 终端直接启动 root 登录式 Bash，默认工作目录
  为 `/root`，可执行完整服务器管理操作。
- 新安装默认启用 Web 终端；配置不再包含 `terminalUser` 和
  `terminalWorkingDirectory`，安装器不再创建隔离用户或依赖 `setpriv/getent/nologin`。
- 保留面板密码二次认证、30 秒一次性票据、来源会话绑定、同源 WebSocket、Origin 校验、
  会话时长、空闲超时、全局/单用户并发限制、进程资源上限、输出上限和命令提交事件审计。
- 前端入口更名为“服务器终端”，明确展示完整 Root 权限和高权限风险，不再显示低权限、
  capability 清除或隔离环境说明。
- 状态接口新增 `available` 与 `privilegeMode: root`；暂时保留
  `isolationAvailable` 兼容旧 WebUI，避免升级过程中按钮被错误禁用。

验证与部署：

- 新增 Root 命令构造测试，确认启动参数不再包含 `setpriv`、`--no-new-privs`、UID/GID
  降权或 capability 清除参数。
- Panel 全量 `go test ./...`、`go vet ./...`、安装器语法检查通过；前端 TypeScript
  类型检查、3 个测试文件共 8 个测试及生产构建通过，嵌入 WebUI 校验通过。
- Panel `v0.1.0-test.46` 已部署到 `192.168.1.6:8089`，服务为 active，健康检查和
  局域网首页均返回 HTTP 200，部署后无 warning 级日志。
- 测试机通过真实 PTY 启动验证 `uid=0(root) gid=0(root)`、工作目录 `/root`；旧
  `one-terminal` 用户、用户组和目录已删除。
- 部署前 `test.45` 二进制、配置和 SQLite 数据库保存在
  `/usr/local/one/backups/manual-release-v0.1.0-test.46`。

风险：

- Web 终端现在拥有完整服务器权限。生产环境必须限制面板来源 IP，并启用 HTTPS、MFA、
  强密码和严格会话策略；任何面板账户失陷都可能导致整台服务器被完全控制。

### 2026-08-07 Root 终端显示增强

状态：Shell 配色、动态路径、终端工具栏、生产构建和测试环境部署已完成。

已完成：

- Root Bash 启用 `xterm-256color`，自动加载 `dircolors`；`ls` 默认按目录优先
  排序，并区分目录、可执行文件、压缩包、链接和普通文件颜色；目录、可执行文件和链接还会
  分别追加 `/`、`*`、`@` 类型标记。
- 修复服务器 `/root/.bashrc` 覆盖 Panel 提示符和颜色设置的问题。每个会话使用权限为 `0600`
  的临时专用 Bash 初始化文件，先加载系统配置，再强制应用 Panel 的最终配色、别名和路径同步
  钩子；会话结束后立即删除该文件。
- 提示符分别显示彩色的 root 身份、主机名、当前路径和命令提示符，Shell 每次显示提示符时
  都会通过终端标题事件同步当前工作目录。
- 终端顶栏新增身份和连接状态、动态路径标签、复制选中内容、清空显示和全屏按钮；底部新增
  文件类型颜色图例、连接状态、UTF-8 与终端类型信息。
- 重做暗色终端调色板、边框、阴影、滚动条、间距和响应式布局，保持与面板主题色光标联动。

验证与部署：

- 后端终端策略测试与 SSH 路由测试通过，新增断言覆盖 256 色、`dircolors`、彩色 `ls`
  和彩色路径提示符；前端 TypeScript 类型检查与生产构建通过。
- 嵌入 WebUI SHA-256 为
  `a0e414b581ab042d590aec1284a054f1d95822d5f5da959f3877af0e87471fa1`。
- Panel `v0.1.0-test.48` 已部署到 `192.168.1.6:8089`；服务为 active，ready 检查中数据库和
  WebUI 均为 `ok`，部署后二进制 SHA-256 为
  `f5ae9194c169d99b15fdcf591c0be4d90a068707a48bd243c52006eff3b9b5f7`。
- 测试机真实 Root PTY 已确认 `ls` 强制别名生效；进入 `/home` 后提示符和终端标题同步为
  `/home`，目录 `sky/` 输出 ANSI 蓝色，RPM/DEB 输出 ANSI 红色并正确分列。
- 部署前 `test.47` 二进制保存在
  `/usr/local/one/backups/manual-release-v0.1.0-test.48/one`。

### 2026-08-07 Root 终端 ANSI 配色兼容严格 CSP

状态：根因修复、生产构建和测试环境部署已完成。

已完成：

- 确认服务端 PTY 已正确输出 ANSI 颜色，页面仍显示灰白色的原因是 Panel 的
  `style-src 'self'` CSP 拒绝了 xterm DOM 渲染器运行时创建的内联颜色样式。
- 保留严格 CSP，不增加 `unsafe-inline`；前端改为在构建阶段生成完整的
  `xterm-256color` 前景色和背景色规则，覆盖基础 16 色、216 色立方体和 24 级灰阶。
- 将 xterm 最低对比度调整关闭为调色板原值，避免渲染器再次使用被 CSP 拒绝的运行时
  内联颜色；补充静态粗体、弱化文字和光标样式。
- Root Shell 不再声明 `COLORTERM=truecolor`，避免命令行程序选择依赖内联 RGB 样式的
  24 位真彩色输出；继续使用兼容且足以区分文件类型的 256 色模式。

验证与部署：

- SSH 策略单元测试、前端 TypeScript 类型检查和生产构建通过；构建产物已检查包含
  `.xterm-fg-45`、`.xterm-fg-245`、`.xterm-bg-196` 和反色规则。
- 嵌入 WebUI SHA-256 为
  `889b78871f38339b0e27e671ad9d6eeee95cc5b77b625809b7ff77af3ee8013b`。
- Panel `v0.1.0-test.50` 已部署到 `192.168.1.6:8089`；服务为 active，live/ready
  均返回 `ok`，部署后二进制 SHA-256 为
  `32b64cde6c06a5649c4992ba5244ecb34622a4985da30a936aa73b6772b98603`。
- 测试服务器实际返回的终端 CSS 已确认包含编译期 ANSI 规则，同时响应头仍保持
  `style-src 'self'`，未降低安全策略。
- 部署前 `test.49` 二进制保存在
  `/usr/local/one/backups/manual-release-v0.1.0-test.50/one`。

### 2026-08-08 Root 终端分栏和空格对齐

状态：排序、字符网格、生产构建和测试环境部署已完成。

已完成：

- 修复严格 CSP 拒绝 xterm 动态基础样式后，终端连续空格被浏览器压缩为一个空格、
  GNU `ls` 分栏因此挤在一起的问题。
- 在前端编译期补充 `white-space: pre`、等宽字体、固定字号、字符块
  `inline-block`、字符高度和垂直对齐规则，完整保留终端原始列宽和填充空格。
- `ls` 取消“目录优先”重排，恢复 GNU 默认名称排序；显式使用纵向分栏和 8 字符制表位，
  让项目按列从上到下排列，与常见 Linux 终端输出一致。
- `ll` 保留单行详细信息模式，`la` 使用与普通 `ls` 一致的名称排序和纵向分栏。

验证与部署：

- SSH 策略测试、WebUI 嵌入测试、前端 TypeScript 类型检查和生产构建通过；生产 CSS
  已确认包含 `white-space:pre` 和 `.xterm-rows span{display:inline-block}`。
- 测试服务器用 120 列真实 GNU `ls` 验证 OneinStack 目录，文件按名称纵向填充为三行，
  各列空格和颜色均正确保留。
- 嵌入 WebUI SHA-256 为
  `874b4531b970c17cda5af11074aae8547341021197fd8301ffe51db935a4f65f`。
- Panel `v0.1.0-test.52` 已部署到 `192.168.1.6:8089`；服务为 active，ready 检查中
  数据库和 WebUI 均为 `ok`，部署后二进制 SHA-256 为
  `a8578c39d563e265cae6704fa2306452da4b0e8b3d2540ecb115a3ef77e4928b`。
- 部署前 `test.51` 二进制保存在
  `/usr/local/one/backups/manual-release-v0.1.0-test.52/one`。

### 2026-08-08 Root 终端说明改为轻量提示

状态：界面调整、生产构建和测试环境部署已完成。

已完成：

- 移除长期占用页面空间的四列会话状态卡片和蓝色审计说明区域。
- 在终端顶栏新增 `Root 权限` 与 `会话审计` 两个轻量圆角提示标签，悬停时再展开
  运行身份、会话限制、并发限制、高权限风险和审计范围。
- 终端不可用的异常警告仍保留为常驻提示，确保真正影响使用的问题不会被隐藏。
- 新提示支持主题变量、键盘焦点和移动端自适应，不改变终端连接与二次认证流程。

验证与部署：

- 前端 TypeScript 类型检查、生产构建、WebUI 嵌入测试和 SSH 策略测试通过。
- 嵌入 WebUI SHA-256 为
  `d9ae43cbfa0fd8226a2025bab7faa320922da532177a21ccbe103c2e7fa0ad4c`。
- Panel `v0.1.0-test.53` 已部署到 `192.168.1.6:8089`；服务为 active，ready 检查中
  数据库和 WebUI 均为 `ok`，测试机实际静态资源已确认包含两个提示标签和对应样式。
- 部署后二进制 SHA-256 为
  `5c27f4be74cbb2b117619addd95c6351adf36505716f3c33fb583b4981483a09`；部署前
  `test.52` 二进制保存在 `/usr/local/one/backups/manual-release-v0.1.0-test.53/one`。

### 2026-08-08 Root 终端可读指令审计

状态：后端采集、敏感信息脱敏、查询、前端展示、自动化验证和测试环境部署已完成。

已完成：

- 终端 `terminal.command.submit` 审计事件不再只写入字符数，改为保存规范化后的可读指令，
  操作者、来源 IP、会话 ID、提交时间继续由原有只追加审计链保存。
- 新增密码、Token、API Key、Authorization、`sshpass`、Redis/MySQL 命令行密码、
  `curl --user`、URL 凭据以及 `chpasswd`/`htpasswd` 等常见敏感内容脱敏规则。
- 每次读取终端输入前检查 PTY `ECHO` 标志；密码提示等无回显状态下不解析、不保存输入，
  并清空尚未提交的缓存行。非 Linux 构建采用失败关闭策略，不记录无法确认可见性的输入。
- 审计全文查询新增 `message` 字段，支持直接搜索终端指令；前端审计表新增“说明 / 指令”列、
  `PTY` 方法筛选和指令搜索提示，无需进入详情抽屉即可确认命令内容。
- 终端页说明同步更新为“保存脱敏后的指令，不保存交互式密码输入和终端输出”。

验证：

- Panel 全量 Go 测试和 `go vet ./...` 通过；SSH 与审计模块测试覆盖普通指令保存、UTF-8
  长度限制、常见凭据脱敏、无回显输入丢弃和按指令内容查询。
- Linux amd64 上的真实 PTY 测试确认 `ECHO` 开启时允许采集、关闭后立即停止采集。
- 前端 TypeScript 类型检查、3 个 Vitest 文件共 8 项测试和生产构建通过；嵌入 WebUI
  SHA-256 为 `84fcaf7c2b553dc5f8527dcebee1ee47debfb073a74a5c960dc50e2e9a9adddb`。
- Panel `v0.1.0-test.54` 已部署到 `192.168.1.6:8089`；服务为 active，live/ready
  检查和局域网首页均正常，生产静态资源确认包含“说明 / 指令”列、`PTY` 筛选和新审计说明。
- 部署后二进制 SHA-256 为
  `219cf6c2e33da334a12f4a20a3b3481efa68958dce75f51cff7eccdfe67b4521`；部署前
  `test.53` 二进制、配置和 SQLite 数据库保存在
  `/usr/local/one/backups/manual-release-v0.1.0-test.54`。

说明：

- 旧审计事件原本没有保存命令内容，无法从历史数据补回；本功能只对升级后新提交的指令生效。
- 脱敏规则覆盖常见凭据形式，但生产运维仍应避免在命令行直接填写秘密，优先使用无回显输入、
  权限受限的凭据文件或环境注入机制。

### 2026-08-09 首页网络流量与磁盘 IO 平滑曲线

状态：前端实现、自动化验证、生产 WebUI 构建和测试环境部署已完成。

已完成：

- 首页网络流量和磁盘 IO 共用的实时趋势图由折线改为平滑单调曲线，使用 `x` 轴单调约束，
  避免流量快速变化时出现尖锐折角、负值回弹或不自然过冲。
- 两组曲线增加圆角粗线、圆形采样点、悬停放大和更新过渡动画，保留原有上行/下行、
  读取/写入配色与数据单位。
- 面积填充改为从曲线向底部逐渐透明的三段渐变，继续保留虚线网格和现有主题适配。
- 实时数据窗口限制为最近 60 个采样点，时间、上行/读取和下行/写入序列同步裁剪，避免页面
  长时间打开后数据与圆点无限累积。

验证：

- 前端 TypeScript 类型检查、3 个 Vitest 文件共 8 项测试和生产构建通过。
- 新嵌入 WebUI SHA-256 为
  `a178db480218854c36de948c4ca7114cbfdd538c6c982cd562e4b6838ae2d3d7`，Panel 的
  WebUI 嵌入测试通过。
- Panel `v0.1.0-test.55` 已部署到 `192.168.1.6:8089`；服务为 active，ready 检查中
  数据库和 WebUI 均为 `ok`，测试服务器实际静态资源已确认包含 `smoothMonotone: x` 和
  `cubicOut` 更新动画配置。
- 部署后二进制 SHA-256 为
  `894215f2a7436139eb1a05d687e7c7311afac6c4671c60ef99e4518fd4b00666`；部署前
  `test.54` 二进制、配置和 SQLite 数据库保存在
  `/usr/local/one/backups/manual-release-v0.1.0-test.55`。

说明：

- 浏览器自动检查已确认测试地址可达，但独立检查会话停留在登录页；请使用已有管理员会话
  强制刷新首页，观察网络流量和磁盘 IO 的实际平滑曲线效果。

### 2026-08-09 文件管理高密度工作台重构

状态：界面重构、自动化验证、生产构建和测试环境部署已完成。

已完成：

- 文件页改为自然高度的统一工作台，移除绝对定位和大块空白；目录标签压缩为 45px，支持多目录标签、关闭和新增。
- 路径导航、返回、刷新、当前目录筛选和深度搜索合并为紧凑导航栏；路径仍支持点击切换为手工输入。
- 上传/下载、新建、收藏夹、目录树和终端集中到单行命令栏；操作记录、回收站和列表/网格视图使用独立快捷入口。
- 增加磁盘使用率、可写空间、文件类型、收藏/软链接标识，以及目录、文件、隐藏项和筛选结果统计。
- 列表默认每页从 10 项提高到 50 项，表头 41px、数据行 45px；操作按钮继续只在鼠标悬停或键盘聚焦时出现，触屏设备始终可见。
- 新增自适应网格视图，保留打开、预览、编辑、下载、收藏、属性和删除入口；窄屏下导航、工具栏、统计区自动换行。
- 通用表格组件新增可选分页摘要插槽，其他页面不传入时保持原有布局。

验证与部署：

- 前端 TypeScript 类型检查、4 个 Vitest 文件共 10 项测试、生产构建和 `git diff --check` 通过。
- 新 WebUI 已同步到 Panel 嵌入包，`go test ./webui` 通过。
- Panel `v0.1.0-test.58` 已部署到 `192.168.1.6:8089`；systemd 服务为 active，
  `/health/ready` 返回数据库和 WebUI 均为 `ok`。
- 部署后二进制 SHA-256 为
  `1cb38412ef80cc6bd1ec6a2e524fc6f361b484ef51c329397dae2b09510c70c3`；部署前版本保存在
  `/usr/local/one/backups/manual-release-v0.1.0-test.58/one`。
