...
func (a *Agent) executeExtendedTask(ctx context.Context, task *models.ClusterTask) (json.RawMessage, error) {
	...
	case "software.install":
		...
		_, err := softwareService.RunInstall(&input.InstallParams{Key: p.Key, Version: p.Version, Port: p.Port, Username: p.Username, Pwd: p.Password, Parameters: p.Parameters})
		return json.Marshal(map[string]any{"key": p.Key, "installed": err == nil})
	case "software.uninstall":
		...
		_, err := softwareService.Remove(&input.RemoveParams{Name: p.Name, Version: p.Version, DataPolicy: p.DataPolicy, ConfirmDataDeletion: p.ConfirmDataDeletion, Parameters: p.Parameters})
		return json.Marshal(map[string]any{"name": p.Name, "uninstalled": err == nil})
