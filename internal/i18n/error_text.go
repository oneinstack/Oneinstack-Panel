...
var englishErrorTexts = map[string]string{
	...
	"审批单已过期，请重新发起申请":                 "This approval request has expired. Submit a new request.",
	"审批单当前不是待审批状态，请刷新后重试":            "This approval request is no longer pending. Refresh its status and try again.",
	"审批单不存在或已被删除":                    "This approval request does not exist or has been deleted.",
	"请求体必须为 JSON 对象；不填写审批意见时请传入 {}。": "The request body must be a JSON object. Send {} when no review comment is needed.",
	"该审批单已超过有效期，无法继续处理。请重新发起审批申请。":            "This approval request has passed its validity period and cannot be processed. Submit a new request.",
	"该审批单可能已被其他人员处理。请刷新页面以获取最新状态。":            "This approval request may have been handled by another user. Refresh the page to get its latest status.",
	"请刷新审批列表后重新选择需要处理的审批单。":                   "Refresh the approval list and select the request again.",
	"审批处理未完成。请刷新审批单确认当前状态；如仍失败，请联系管理员检查服务日志。": "The approval was not completed. Refresh the request to confirm its current status; if the issue continues, ask an administrator to check the service logs.",
	...
	"数据库连接超时":                               "The database connection timed out",
	"数据库连接被拒绝":                              "The database connection was refused",
	...
	"目标数据库拒绝连接，请确认服务已启动、监听地址和端口配置正确。":                                                         "The target database refused the connection. Confirm that the service is running and that the listening address and port are correct.",
	"无法解析目标数据库地址，请检查地址和 DNS 配置后重试。":                                                           "The target database address could not be resolved. Check the address and DNS configuration before retrying.",
	"面板服务器当前无法访问目标数据库网络，请检查路由、安全组和防火墙配置。":                                                     "The Panel server cannot currently reach the target database network. Check the routing, security-group, and firewall configuration.",
	"目标数据库已响应，但用户名或密码未通过认证，请核对登录凭据。":                                                          "The target database responded, but the username or password was not accepted. Check the login credentials.",
	"无法连接到目标数据库，请检查地址、端口、登录凭据和网络访问策略后重试。":                                                     "The target database could not be reached. Check the address, port, login credentials, and network access policy before retrying.",
	"恢复前安全备份失败：当前数据库账号没有导出所需权限，恢复操作尚未开始，原数据库未修改。请检查 SELECT、SHOW VIEW、TRIGGER、EVENT 等权限后重试。":   "The pre-restore safety backup failed: the database account lacks the privileges required for export. The restore has not started and the original database was not modified. Check SELECT, SHOW VIEW, TRIGGER, EVENT, and related privileges, then retry.",
	"数据库备份失败：当前数据库账号没有导出所需权限，请检查 SELECT、SHOW VIEW、TRIGGER、EVENT 等权限后重试。数据库内容不会因本次备份失败被删除或修改。": "Database backup failed: the database account lacks the privileges required for export. Check SELECT, SHOW VIEW, TRIGGER, EVENT, and related privileges, then retry. The database content is not deleted or modified by a failed backup.",
	"数据库恢复失败：当前数据库账号没有恢复所需权限，请检查账号权限后重试；恢复前安全备份已保留。":                                          "Database restore failed: the database account lacks the privileges required for restore. Check the account privileges and retry. The pre-restore safety backup has been retained.",
	"数据库备份失败：未找到 mysqldump 客户端，请安装 MySQL 客户端并确认组件路径或 PATH 配置后重试。":                             "Database backup failed: the mysqldump client was not found. Install the MySQL client and check the component path or PATH configuration, then retry.",
	"数据库恢复失败：未找到 mysql 客户端，请安装 MySQL 客户端并确认组件路径或 PATH 配置后重试。":                                 "Database restore failed: the mysql client was not found. Install the MySQL client and check the component path or PATH configuration, then retry.",
	"恢复前安全备份失败：未找到 mysqldump 客户端，恢复操作尚未开始，原数据库未修改。请安装 MySQL 客户端并确认组件路径或 PATH 配置后重试。":          "The pre-restore safety backup failed: mysqldump was not found. The restore has not started and the original database was not modified. Install the MySQL client and check the component path or PATH configuration, then retry.",
	"恢复前安全备份失败：备份目录可用空间不足，恢复操作尚未开始，原数据库未修改。请释放磁盘空间后重试。":                                       "The pre-restore safety backup failed: the backup directory does not have enough free space. The restore has not started and the original database was not modified. Free disk space and retry.",
	"数据库备份失败：备份目录可用空间不足，数据库内容不会因本次备份失败被删除或修改。请释放磁盘空间后重试。":                                     "Database backup failed: the backup directory does not have enough free space. The database content is not deleted or modified by a failed backup. Free disk space and retry.",
	"恢复前安全备份失败：无法创建或发布安全备份文件，恢复操作尚未开始，原数据库未修改。请检查备份目录权限和磁盘状态后重试。":                             "The pre-restore safety backup failed: the safety-backup file could not be created or published. The restore has not started and the original database was not modified. Check the backup-directory permissions and disk status, then retry.",
	"数据库备份失败：无法创建或发布备份文件，数据库内容不会因本次备份失败被删除或修改。请检查备份目录权限和磁盘状态后重试。":                             "Database backup failed: the backup file could not be created or published. The database content is not deleted or modified by a failed backup. Check the backup-directory permissions and disk status, then retry.",
	"数据库备份失败：无法保存备份记录，未登记的备份文件已清理。数据库内容不会因本次备份失败被删除或修改。请检查 Panel 数据库状态和任务日志后重试。":              "Database backup failed: the backup record could not be saved, and the unregistered backup file was cleaned up. The database content is not deleted or modified by a failed backup. Check the Panel database status and task log, then retry.",
	"数据库恢复失败：恢复源备份不存在、已损坏或与当前数据库不匹配；恢复操作尚未开始，原数据库未修改。请重新生成或选择其他备份。":                           "Database restore failed: the source backup does not exist, is corrupted, or does not match the current database. The restore has not started and the original database was not modified. Generate a new backup or choose another one.",
	"恢复前安全备份登记失败：恢复操作尚未开始，原数据库未修改。请检查 Panel 数据库状态和任务日志后重试。":                                   "The pre-restore safety backup could not be registered. The restore has not started and the original database was not modified. Check the Panel database status and task log, then retry.",
	"恢复前安全备份失败：恢复操作尚未开始，原数据库未修改。请查看任务日志中的具体原因，修正后重试。":                                         "The pre-restore safety backup failed. The restore has not started and the original database was not modified. Check the task log for the specific cause, fix it, and retry.",
	"数据库恢复失败：恢复过程未完成，恢复前安全备份已保留。请查看任务日志确认失败原因，并优先使用安全备份恢复。":                                   "Database restore failed: the restore did not complete, and the pre-restore safety backup has been retained. Check the task log for the cause and restore from the safety backup first.",
	"数据库备份失败：MySQL 导出未完成，数据库内容不会因本次备份失败被删除或修改。请查看任务日志末尾的具体错误，修正后重试。":                          "Database backup failed: the MySQL export did not complete. The database content is not deleted or modified by a failed backup. Check the final error in the task log, fix it, and retry.",
	"数据库恢复失败：请查看任务日志确认失败阶段和具体原因；如果任务已生成恢复前安全备份，请优先使用该备份恢复。":                                   "Database restore failed: check the task log for the failed stage and specific cause. If a pre-restore safety backup was created, restore from it first.",
	"恢复前安全备份失败：恢复操作尚未开始，原数据库未修改。请查看任务日志确认具体原因后重试。":                                            "The pre-restore safety backup failed. The restore has not started and the original database was not modified. Check the task log for the cause and retry.",
	"数据库备份失败：数据库内容不会因本次备份失败被删除或修改。请查看任务日志确认具体原因后重试。":                                          "Database backup failed: the database content is not deleted or modified by a failed backup. Check the task log for the cause and retry.",
	...
	"文件大小超过在线编辑限制，请缩小文件后重试。": "The file exceeds the online editing size limit. Reduce the file size and retry.",
	...
	"上传文件超过允许大小，请选择较小的文件后重试。":   "The uploaded file exceeds the allowed size. Choose a smaller file and retry.",
	"图片超过在线预览大小限制，请选择较小的图片后重试。": "The image exceeds the online preview size limit. Choose a smaller image and retry.",
	"远程文件超过允许大小，请选择较小的文件后重试。":   "The remote file exceeds the allowed size. Choose a smaller file and retry.",
	...
	"用户不存在或已被删除":                                "The user does not exist or was already deleted",
	...
	"网站不存在或已被删除，请刷新后重试":                         "The website does not exist or was removed. Refresh and retry.",
	"网站已到期，请先修改到期时间后再启用":                        "The website has expired. Update its expiration time before enabling it.",
	"网站已存在，请检查网站名称或域名":                          "The website already exists. Check the website name or domain.",
	"网站操作参数格式错误":                                "The website operation payload has an invalid format",
	"网站操作缺少 websiteId 或 id":                     "The website operation must include websiteId or id",
	"网站参数无效":                                    "The website parameters are invalid",
	"网站根目录无效，请填写受管网站根目录下的相对目录":                  "The website root directory is invalid. Enter a relative directory under the managed website root.",
	...
	"配置已发生变化，请重新预览后再执行":                         "The configuration has changed. Create a new preview before executing it.",
	"不支持的网站更新操作":                                "This website update action is not supported",
...
var englishOperationPreviewErrorTexts = map[string]string{
	...
	"当前主机软件源没有请求的 firewalld 版本": "The requested firewalld version is unavailable in this host's package repository",
	"请刷新软件列表并选择本机推荐版本；手动指定的版本不会自动替换。":                           "Refresh the software list and select the host-recommended version. An explicitly requested version will not be replaced automatically.",
	"无法读取主机软件源中的 firewalld 候选版本":                                "Cannot read firewalld candidates from the host package repository",
	"请检查当前系统的软件源并刷新包索引，再重新预览。":                                  "Check this system's package repositories and refresh their metadata, then create a new preview.",
	"无法探测当前主机适用的 firewalld 版本":                                  "Cannot detect a suitable firewalld version for this host",
	"请发布支持主机版本探测的 firewalld 组件包（1.0.5 或以上），刷新软件目录后重试。":          "Publish a firewalld component package with host version discovery (1.0.5 or later), refresh the software catalog, and retry.",
	"请先备份现有规则并确认 SSH、Panel 及业务端口的放行规则，完成防火墙切换后再重试；修改版本无法解决此冲突。": "Back up the existing rules and confirm that SSH, Panel and application ports are allowed before switching firewall backends. Changing the version does not resolve this conflict.",
	"当前主机软件源没有请求的 firewalld 版本，请选择本机推荐版本":                       "The requested firewalld version is unavailable in the host repository. Select a host-recommended version.",
	"无法读取 firewalld 候选版本，请检查主机软件源并刷新包索引":                        "Cannot query firewalld candidates. Check the host repositories and refresh their metadata.",
	"无法探测主机 firewalld 版本，请更新组件包并刷新目录":                           "Cannot discover the host firewalld version. Update the component package and refresh the catalog.",
	"主机 firewalld 版本探测执行失败":                                     "The firewalld host version probe failed to execute",
	"请检查 Panel 运行用户、组件缓存、bash 和主机包管理器后刷新软件列表重试。":                "Check the Panel runtime user, component cache, bash, and host package manager, then refresh the software list and retry.",
