...
配置可信更新公钥和清单地址后，也可以在“设置 → 面板更新”中检查并安装签名版本，或使用：
...
更新会先校验 Ed25519 清单、制品大小和 SHA-256，再执行数据库迁移预检、版本原子切换和健康检查；失败时自动恢复旧二进制、数据库、配置和内置脚本。密钥配置、手工恢复和发布流程见 [BUILD.md](BUILD.md)。
