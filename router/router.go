...
func SetupRouter() *gin.Engine {
	...
	// 公共路由必须显式注册在受保护路由组之外。
	{
		...
		r.POST("/cluster/identity/challenge", middleware.RateLimitMiddleware(30, time.Minute), clusterHandler.ChallengeIdentity)
		r.POST("/cluster/agent/heartbeat", middleware.RateLimitMiddleware(240, time.Minute), clusterHandler.Heartbeat)
		r.POST("/cluster/agent/health", middleware.RateLimitMiddleware(30, time.Minute), clusterHandler.ReportHealth)
		...
		r.POST("/cluster/agent/tasks/control", middleware.RateLimitMiddleware(240, time.Minute), clusterHandler.TaskControl)
	...
	{
		clusterg.GET("/agent/settings", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.GetAgentSettings)
		clusterg.PUT("/role", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.SelectRole)
		clusterg.POST("/role/reset", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.ResetRole)
		clusterg.PUT("/agent/settings", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.UpdateAgentSettings)
		clusterg.GET("/nodes", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.ListNodes)
		clusterg.GET("/health/summary", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.HealthSummary)
		clusterg.GET("/health/resources", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.HealthResources)
		clusterg.GET("/health/resources/:id", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.HealthResourceDetail)
		clusterg.GET("/health/events", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.HealthEvents)
		clusterg.PUT("/health/resources/:id/notification", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.UpdateHealthNotification)
		clusterg.GET("/policy", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.GetPolicy)
		...
		clusterg.POST("/nodes/:id/address-verify", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.VerifyNodeAddress)
		...
		clusterg.POST("/nodes/:id/diagnostics", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.DiagnoseNode)
		clusterg.GET("/panel-updates", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.ListPanelUpdates)
		clusterg.POST("/nodes/:id/lifecycle", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.ChangeNodeLifecycle)
		...
		clusterg.GET("/nodes/:id/tasks/:taskId/events", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.ListTaskEvents)
		clusterg.POST("/nodes/:id/tasks/:taskId/cancel", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.CancelNodeTask)
		clusterg.GET("/nodes/:id/service-actions", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.ListNodeServiceActions)
		clusterg.POST("/service-actions/preview", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.PreviewServiceAction)
		clusterg.POST("/service-actions", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.ExecuteServiceAction)
		clusterg.POST("/batches/preview", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.PreviewBatch)
		clusterg.POST("/batches", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.CreateBatch)
		clusterg.GET("/batches", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.ListBatches)
		clusterg.GET("/batches/:id", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.GetBatch)
		clusterg.POST("/batches/:id/cancel", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.CancelBatch)
		clusterg.GET("/tasks", middleware.RequirePermission(accessservice.PermissionClusterRead), clusterHandler.ListAllTasks)
		clusterg.POST("/tasks", middleware.RequireSuperAdmin(), clusterHandler.EnqueueTask)
		clusterg.POST("/website/dispatch", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.DispatchWebsite)
		clusterg.DELETE("/nodes/:id", middleware.RequirePermission(accessservice.PermissionClusterWrite), clusterHandler.DeleteNode)
	}
	...
	softg := protected.Group("/soft")
	{
		...
		softg.POST("/install/offline", middleware.RequirePermission(accessservice.PermissionSoftwareWrite), software.RunOfflineInstallation)
		...
		softg.GET("/tasks/:id/secret", middleware.RequirePermission(accessservice.PermissionSoftwareRead), middleware.RequireAnyPermission(accessservice.PermissionTaskReadSelf, accessservice.PermissionTaskReadAll), software.GetSoftwareTaskSecret)
